...
def split_table_text_logically(table_data):
    ...
    for row in table_data:
        ...
        row_text = "\t".join(row)
        if (
            "Конец таблицы, начало пояснений" in row
        ):  # Проверяем, встречается ли 'Cir73SPb+' в строке
            print(
                "🔴 Обнаружен 'Конец таблицы, начало пояснений'. Прерываем обработку таблицы."
            )
            count = False
        if count == False:
            continue
...
def save_table_to_minio(bucket_name, table_name, table_data):
    ...
    for row_idx in range(1, max_rows + 1):
        ...
        # Устанавливаем высоту строки: 15 после появления 'Конец таблицы, начало пояснений'
        if found_target:
